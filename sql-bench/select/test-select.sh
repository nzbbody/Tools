...
if ($limits->{'group_functions'})
{
  ...
  fetch_all_rows($dbh,"select sql_no_cache sum(idn+$tmp),sum(rev_idn-$tmp) from bench1");

  $loop_time=new Benchmark;
  for ($tests=0 ; $tests < $opt_loop_count ; $tests++)
  {
    fetch_all_rows($dbh,"select sql_no_cache sum(idn+100),sum(rev_idn-100) from bench1");
  }
  $end_time=new Benchmark;
